...
from net.networkCBFading import WITT
...
parser.add_argument('--lambda_loss', type=float, default='0.01',
                    help='lambda in the loss function, 0.01, 0.1')
                ...
                if batch_idx % 50 == 0 and epoch % 5 == 0:
                    ...
                    for iii in range(input.size()[0]):
                        if args.channel_type == 'awgn':
                            save_image(recon_image[iii], ('./image_recover_CB/img%d_epoch%d_batch%d_snr%d.png' % (iii, epoch, batch_idx, snr))) 
                        else:
                            save_image(recon_image[iii], ('./image_recover_CB_Fading/img%d_epoch%d_batch%d_snr%d.png' % (iii, epoch, batch_idx, snr)))     
...
if __name__ == '__main__':
    ...
    for output_dir in (
            './results_data', './results_data/results_CB', './results_data/results_CB_Fading',
            './saved_model/awgn/STL10', './saved_model/rayleigh/STL10',
            './image_recover_CB', './image_recover_CB_Fading'):
        makedirs(output_dir)
    ...
    # load the codebook
    codebook_np = np.load('./results_data/codebook_size' + str(args.SCsize) + '.npy')
    ...
    if args.channel_type == 'awgn':
        ...
        model_path = "./saved_model/awgn/STL10/CB_snr" + str(snr_ini) + "_C" + str(args.C) + ".model"
    ...
    if args.training:
        for epoch in range(steps_epoch, config.tot_epoch):          
            ...
            if (epoch + 1) % config.save_model_freq == 0:
                if args.channel_type == 'awgn':
                    save_model(net, save_path='./saved_model/awgn/STL10/CB_snr{}_C{}.model'.format(snr, args.C))
                else:
                    save_model(net, save_path='./saved_model/rayleigh/STL10/CB_snr{}_C{}.model'.format(snr, args.C))
                ...
                if args.channel_type == 'awgn':
                    channel_str = 'results_CB'
                else:
                    channel_str = 'results_CB_Fading'
