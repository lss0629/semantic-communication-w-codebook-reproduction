                ...
                if batch_idx % 50 == 0 and epoch % 5 == 0:
                    ...
                    for iii in range(input.size()[0]):
                        if args.channel_type == 'awgn':
                            save_image(recon_image[iii], ('./image_recover_SC/img%d_epoch%d_batch%d_snr%d.png' % (iii, epoch, batch_idx, snr))) 
                        else:
                            save_image(recon_image[iii], ('./image_recover_SC_Fading/img%d_epoch%d_batch%d_snr%d.png' % (iii, epoch, batch_idx, snr))) 
...
if __name__ == '__main__':
    ...
    for output_dir in (
            './results_data', './results_data/results_SC', './results_data/results_SC_Fading',
            './saved_model/awgn/STL10', './saved_model/rayleigh/STL10',
            './image_recover_SC', './image_recover_SC_Fading'):
        makedirs(output_dir)
    ...
    if args.channel_type == 'awgn':
        ...
        model_path = "./saved_model/awgn/STL10/SC_snr" + str(snr_ini) + "_C" + str(args.C) + ".model"
    ...
    if args.training:
        for epoch in range(steps_epoch, config.tot_epoch): 
            ...
            if (epoch + 1) % config.save_model_freq == 0:
                if args.channel_type == 'awgn':
                    save_model(net, save_path='./saved_model/awgn/STL10/SC_snr{}_C{}.model'.format(snr, args.C))
                else:
                    save_model(net, save_path='./saved_model/rayleigh/STL10/SC_snr{}_C{}.model'.format(snr, args.C))
                ...
                if args.channel_type == 'awgn':
                    channel_str = 'results_SC'
                else:
                    channel_str = 'results_SC_Fading'
