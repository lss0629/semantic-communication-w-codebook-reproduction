...
class WITT_Encoder(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans,
        ...
        # fusion the image and the codeword
        self.tconv1 = nn.ConvTranspose2d(3, 3, kernel_size=3, stride=1, padding=0)
        self.conv1 = nn.Conv2d(3, 3, kernel_size=3, stride=1, padding=0)
        self.tconv2 = nn.ConvTranspose2d(3, 3, kernel_size=3, stride=1, padding=0)
        self.conv2 = nn.Conv2d(3, 3, kernel_size=3, stride=1, padding=0)
        self.tconv3 = nn.ConvTranspose2d(3, 3, kernel_size=3, stride=1, padding=0)
        self.conv3 = nn.Conv2d(3, 3, kernel_size=3, stride=1, padding=0)


        self.FCMean = torch.nn.Linear(int(256*C), int(256*C))
        self.FCVar = torch.nn.Linear(int(256*C), int(256*C)) 
    ...
    def forward(self, x, cb, snr, model):
        B, C, H, W = x.size()
        device = x.get_device()
        ...
        B, L, C = x.size()
        # get the mean vector and the log_variance vector
        x  = x.view(B, L*C).clone()
        xlog_var = x.clone()

        # get the mean vector
        x = self.FCMean(x)

        # get the log_variance vector
        xlog_var = self.FCVar(xlog_var)

        # generate the Gaussian variable
        std = torch.exp(0.5 * xlog_var)
        esp = torch.randn(x.size()).to(device) 
        x_Gaussian = torch.add(x, torch.mul(std, esp)).clone()  
        x_Gaussian = x_Gaussian.view(B, L, C)  # transform to the original shape

        return x_Gaussian, x, std
